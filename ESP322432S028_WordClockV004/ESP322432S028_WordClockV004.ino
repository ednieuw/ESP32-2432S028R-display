...
--------------------------------------  
 ...
 Changes V012: Using ledcSetup(ledChannel, freq, resolution); to remove display flickering
               timeinfo.tm_mon starts at januari = 0 
               Menu entry & forces NTP update
               Digital display on screen updated
 Changes V013: Analog clock added as option
 Changes V014: Redecorated the analog clock
 Changes V015: Built analogue clock
 Changes V016: Built analogue clock. Corrected month in NTP. month 0 is janyary
 Changes V017: Stable
 Changes V018: SetStatusLED ESP32-s3
 Changes V019: Added WiFi.onEvents for connection and error coes and proper connection handling.
               WIFI.Events worked fine but ....
               Discontinued with ILI9341. Using WiFi.onEvent interrupts and crashes screen display while writing the screen
 Changes V020: Added OTA. This version compiles and runs on ESP32-C3. 
 Changes V021: Develop/Testing. Mem menory expanded. restart ESP32 once after first boot when no IP-addreess cq connection. 
 Changes V022: Works with SP32-C3. OTA fails with 2MB esp32-C3. Too less memory
 Changes V023: Works with SP32-C3.
 Changes V024: Corrected some compiler warnings 
 Changes V025: Isolate display functions to make it easy to add another display. Optimized lockup during printing to ILI9341 display
 Changes V026: If WIFI connection is lost reconnect every hour. Turn off Red LED after startup if Display LEDs is off
 Changes V027: Solved analog clock outer ring not shown after display off -> Added void RestartDisplay(void)
 Changes V028: Hour hand nicer. 
...
Set to:
Board: ESP32-C3 DEV module or ESP32-S3 DEV module
Flash size 4MB ot 2MB if you have a cheaper 2MB board.
OTA will not wotk on 2MB boards
...
#define OCLOCK  ColorLeds("O'clock",122, 127, LetterColor);
...
void EverySecondCheck(void)
{
 ...
 if (msLeap >999)                                                                             // Every second enter the loop
  { 
   msTick = millis();
   if (msLeap%500==0  && Mem.StatusLEDOnOff)                                                  // Do not update the LED unnecesary often.It takes a lot of msecs per update
        SetStatusLED(0,0,6,6*WIFIConnected,6*deviceConnected);     
...
{ 
 ...
 for (n=FirstLed; n<=FirstLed + (LastLed-FirstLed); n++)
 {
  ...
  sprintf(sptext,"-Strippos[%d].Character=:%c",n, Strippos[n].Character); Tekstprintln(sptext);   
 }
}
//                                                                                            //
//--------------------------------------------
 ...
 switch(Language)                                                                             //
     {
      ...
      case 1: 
      strncpy(Template,"HITVISOWASOEXACTHALFITWENTYEFIVEQUARTERTENZPASTATOLFSMIDKSIXVTWOFIVEETWELVEXTENRELEVENENFOURAONETEIGHTHREENIGHTFNINESEVENROCLOCK",128);  
              ColorLeds(Template,0,127, Mem.DimmedLetter);
...
void DrawAnalogeClockHands(void)
{
 ...
 DrawSecondHand(ss,     smx, smy, 0xF800);    // RED                                                         // Erase previous text                                       
